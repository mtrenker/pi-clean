# Task: Add backward-compatible tests and fixtures for old and new fleet formats
...
## Requirements
Use the implementation outputs from Tasks 002 through 007 to add regression tests and fixtures under `extensions/fleet/` for run metadata, git-context fallback, event writing, usage normalization, heartbeat updates, attention derivation, archive metadata, and old-format compatibility. The tests should cover both fresh runs and historical task folders missing new fields, and acceptance requires `node --test extensions/fleet/config.test.ts extensions/fleet/plan.test.ts extensions/fleet/state.test.ts extensions/fleet/task.test.ts extensions/fleet/widget.test.ts extensions/fleet/engines/codex.test.ts` to pass or to document any pre-existing unrelated failures clearly in a handoff note.

## Workspace Rules
- Work only inside the current working directory.
- Use relative paths from cwd; do not assume absolute paths like `/root/project`.
- Your task instructions are in `.pi/tasks/008-add-backward-compatible-tests-and-fixtures-for-old-and-new-fleet-formats/task.md`.
- Write progress updates only to `.pi/tasks/008-add-backward-compatible-tests-and-fixtures-for-old-and-new-fleet-formats/progress.jsonl`; never create or append to a repo-root `progress.jsonl`.
- Raw engine output is captured separately in `.pi/tasks/008-add-backward-compatible-tests-and-fixtures-for-old-and-new-fleet-formats/output.jsonl`.
- Prefer targeted searches with exclusions (exclude `node_modules`, `.git`, and `.pi/archive` unless the task explicitly needs them).
- Avoid broad repo-wide scans such as `**/*.md` when a narrower path or pattern will do.
- If you already have enough context, stop exploring and produce the deliverable.

## Progress Tracking
Append one JSON line to `.pi/tasks/008-add-backward-compatible-tests-and-fixtures-for-old-and-new-fleet-formats/progress.jsonl` after each significant step:
{"ts":"<ISO timestamp>","step":"<description>","status":"done"|"running"|"error"}

## Completion Handoff
Before finishing, write `.pi/tasks/008-add-backward-compatible-tests-and-fixtures-for-old-and-new-fleet-formats/handoff.md` with:
- changed files
