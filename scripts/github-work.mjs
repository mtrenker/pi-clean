...
function issueAgentPrompt(agent, number, repository) {
  ...
  return `${task} Do not originate or materially change unresolved product, UX, interaction, visual, architecture, API, or data-model design. If such design is required and is not already approved, stop and report the required Claude Opus 5.5 handoff.`;
}
...
function reviewAgentPrompt(reviewer, number, repository) {
  ...
  return `${task} Review implementation fidelity against approved design, but do not make final judgments on unresolved product, UX, interaction, visual, architecture, API, or data-model design; flag those for Claude Opus 5.5.`;
}
