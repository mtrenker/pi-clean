...
Every profile also disables the vendor's own worker spawning at every effort level, and every managed prompt repeats that boundary in words: Codex passes `--disable multi_agent`, Claude passes `--settings '{"disabledBuiltinTools":["Task"]}'`, and Pi ships no sub-agents. These are boundaries, not guarantees. Each session still has a shell, the flags are not verified end to end, and neither profile sandboxes the host. A detached worktree isolates Git state rather than untrusted repository code; stronger sandboxing is separate follow-up work.
