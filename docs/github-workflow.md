...
Every rendered launch appends a delegation boundary to its prompt, and the vendor profiles add a flag at every effort level: Codex passes `--disable multi_agent`, Claude passes `--disallowed-tools Task`. `pi-ambient` has no such flag, because Pi loads extensions from personal settings that this repository does not control. These are boundaries, not guarantees. Each session still has a shell, the flags are not verified end to end, and neither profile sandboxes the host. A detached worktree isolates Git state rather than untrusted repository code; stronger sandboxing is separate follow-up work.
